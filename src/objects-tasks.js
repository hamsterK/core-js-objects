...
function makeWord(lettersObject) {
  ...
  Object.entries(lettersObject).forEach(([key, value]) => {
    value.forEach((position) => {
      reversedObject[position] = key;
    });
  });
  return Object.values(reversedObject).join('');
}
...
function sellTickets(queue) {
  ...
  queue.forEach((bill) => {
    if (bill === 25) {
      ...
      if (wallet.bill50 >= 1 && wallet.bill25 >= 1) {
        wallet.bill50 -= 1;
        wallet.bill25 -= 1;
        wallet.bill100 += 1;
      } else if (wallet.bill25 >= 3) {
        wallet.bill25 -= 3;
        wallet.bill100 += 1;
      } else {
        result = false;
...
function Rectangle(width, height) {
  class Rect {
    constructor(widthParam, heightParam) {
